// 初始化上传图片组件
var imageWebUploader = function(conf) {

    const template = 
        '<div id="uploader" class="web-uploader wu-example">'+
            '<div class="queueList">'+
    ...
    var $wrap = $container.find('.web-uploader'),
    ...
    uploader = WebUploader.create({
        ...
        //上传文件名
        fileVal:'cover',
        fileNumLimit: conf.fileNumLimit,
        fileSizeLimit: conf.fileSizeLimit,    // 1 M
        fileSingleSizeLimit: conf.fileSingleSizeLimit    // 1 M
    });

    // 添加“添加文件”的按钮，
    if (conf.showContinueBtn) {
        uploader.addButton({
            id: conf.container+' #filePicker2',
            label: '继续添加'
        });
    }
        ...
        if ( state === 'ready' ) {
            ...
            if ( stats.uploadFailNum ) {
                text = '已成功上传' + stats.successNum+ '张图片，'+
                    stats.uploadFailNum + '张照片上传失败，<a class="retry" href="javascript:;">重新上传'//+'</a>失败图片或<a class="ignore" href="#">忽略</a>'
            }
    ...
    uploader.onError = function( code ) {
        if(code == "F_DUPLICATE"){
            ...
            alert("所选附件总大小不可超过"  + conf.fileSizeLimit / 1024 + "KB");
        }else if(code == "Q_EXCEED_NUM_LIMIT") {
            alert("最多只能上传"+conf.fileNumLimit+"张图片。");
        }else{
            alert( 'Eroor: ' + code );
